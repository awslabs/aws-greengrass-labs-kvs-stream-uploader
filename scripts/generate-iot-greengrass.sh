...
if [ $# -ne 5 ]; then
  echo 1>&2 "Usage: $0 AwsProfile IotThingName ComponentName ComponentVersion ComponentArtifactsBucket"
  exit 1
fi
AwsProfile=$1
...
# e.g: aws.greengrass.labs.kvs.stream.uploader
ComponentName=$3
# e.g: 1.0.0
ComponentVersion=$4
...
# the config.yaml for the installation of greengrass core
...
if [ ${#ListThingPrincipalsName[@]} -eq 1 ]; then
  ...
  CertificateArn=$ListThingPrincipalsName
  ...
  read -p "Please select the certificate you want to setup" IndexOfCert
  ...
  CertificateArn=$(jq --raw-output '.certificateArn' $TempPath/certificate)
...
if [ $IamPolicyBasicExisted -eq -1 ]; then
  ...
  aws --profile $AwsProfile iam attach-role-policy --role-name $IamRole --policy-arn $(jq --raw-output '.Policy.Arn' $TempPath/IamRolePolicy.json)
else
  echo "Attaching $IamPolicyBasic to $IamRole"
...
if [ $? -eq 0 ]; then
  ...
  aws --profile $AwsProfile iot create-role-alias --role-alias $IotRoleAlias --role-arn $(jq --raw-output '.Role.Arn' $TempPath/IotRole.json) > $TempPath/IotRoleAlias.json
  IotRoleAliasArn=$(jq --raw-output '.roleAliasArn' $TempPath/IotRoleAlias.json)
fi
...
dirPath=$(dirname $0)
source $dirPath/attach-necessary-policies.sh $AwsProfile $IamRole $ComponentArtifactsBucket
source $dirPath/create-aws-resource.sh $AwsProfile $ComponentArtifactsBucket
source $dirPath/generate-component.sh $AwsProfile $ComponentName $ComponentVersion $ComponentArtifactsBucket
